...
// 				// make sure the thread has at least one message
// 				if (!threadData.messages || threadData.messages.length === 0) continue;

// 				const firstMsg = threadData.messages[0];

// 				// skip if not in inbox
// 				if (!firstMsg.labelIds?.includes("INBOX")) continue;

// 				// extract headers safely
// 				const headers = firstMsg.payload?.headers || [];
// 				const fromHeader = headers.find((h) => h.name.toLowerCase() === "from");
// 				const fromEmail = fromHeader ? fromHeader.value : "(unknown sender)";

// 				console.log("Analyzing message from:", fromEmail);

// 				// analyze message snippet + headers
// 				const curScore = analyzeMsg(firstMsg.snippet, headers);
...
// 				// const emailElement = document.querySelector(`[data-email='${fromEmail}']`);
// 				// highlightEmail(emailElement, curScore);
// 			}
